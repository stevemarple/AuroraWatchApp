...
//	long currentLevelColor = 0L;
	...
	private void showNotification() {
		...
		Notification notification = new Notification(
				R.drawable.ic_stat_notify_aw, text,
	...
	public void run() {
		...
		case STATE_FETCHING_XML:
			currentXml = null;
			httpRequestTask = (HttpRequestTask) new HttpRequestTask().execute(url);
		...
		case STATE_PROCESSING_XML:
			if (currentXml != null) {
				...
				if (previousXml == null || !currentXml.currentState.name.equals(previousXml.currentState.name)) {
					Vibrator v = (Vibrator) getSystemService(Context.VIBRATOR_SERVICE);
					v.vibrate(500);
					Toast.makeText(this, getString(R.string.appName) + ": " + currentXml.currentState.description,
							Toast.LENGTH_LONG).show();
					...
					Intent notificationIntent = new Intent(this, AuroraWatchUKActivity.class);
				...
				try {
					...
					Log.e(getClass().getSimpleName(), "Could not clone currentXml", e);
				...
				Log.i(getClass().getSimpleName(), "Timeout fetching/parsing " + url.toString());
				// Try again, but don't wait for a full update interval
				delay = updateInterval / 2;
			}

			Log.i(getClass().getSimpleName(),
					"run() waiting for " + Long.toString(delay)
							+ " ms");
	...
	class HttpRequestTask extends AsyncTask<URL, Void, AuroraWatchXml> {
		...
		protected AuroraWatchXml doInBackground(URL... params) {
			...
			try {
				...
				Log.e(getClass().getSimpleName(), "Could not fetch " + url.toString(), e);
				return null;
			} 
			...
			try {
				...
				Log.e(getClass().getSimpleName(), "Could not parse " + url.toString(), e);
			} catch (Exception e) {
		...
		private AuroraWatchXml parseStream(InputStream in, String encoding)
			...
			while (eventType != XmlPullParser.END_DOCUMENT) {
				...
				if (eventType == XmlPullParser.START_TAG) {
					...
					if (tag.equals("state")) {
						...
						// state.description = xpp.getAttributeValue(null, "description");
						state.value = Integer.parseInt(xpp.getAttributeValue(null, "value"));
						state.color = Integer.parseInt(xpp.getAttributeValue(null, "color").substring(1), 16);
					}
				}
					...
					if (tag.equals("current")) {
						...
						Log.i(getClass().getSimpleName(), "end tag for " + tag + " state: " + state.toString());
					}
					...
					Log.d(getClass().getSimpleName(), "Found text '" + text + "', tag '" + tag + "'");
					if (tag.equals("state"))
						...
						try {
							...
							Log.e(getClass().getSimpleName(), "Could not parse date '" + text + "'", e);
			...
			Log.i(getClass().getSimpleName() + ".onPostExecute()", 
					"TID=" + Integer.toString(android.os.Process.myTid()) + " " + xmlState.toString());
			return xmlState;
		}
		...
		protected void onPostExecute(AuroraWatchXml s) {
			if (s == null)
				...
				Log.i(getClass().getSimpleName() + ".onPostExecute()", s.toString());
			currentXml = s;
		}
